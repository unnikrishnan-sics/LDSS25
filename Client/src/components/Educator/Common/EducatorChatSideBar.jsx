    ...
    const fetchData = async () => {
      try {
        ...
        const parentsRes = await axios.get(
          `${import.meta.env.VITE_SERVER_URL}/ldss/educator/getapprovedparents/${educatorDetails._id}`,
        ...
        const childrenRes = await axios.get(
          `${import.meta.env.VITE_SERVER_URL}/ldss/educator/getchildrenofallapprovedparents/${educatorDetails._id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
          ...
          const therapistPromises = therapistIdsArray.map(async (therapistId) => {
            try {
              ...
              const therapistRes = await axios.get(
                `${import.meta.env.VITE_SERVER_URL}/ldss/theraphist/gettheraphist/${therapistId}`,
                { headers: { Authorization: `Bearer ${token}` } }
              );
              ...
              >
                ...
                >
                  {therapist.profilePic ? (
                    <Avatar
                      src={`${import.meta.env.VITE_SERVER_URL}/uploads/${therapist.profilePic}`}
                      alt={therapist.name || ''}
                    />
                  ) : (
                    <Avatar
